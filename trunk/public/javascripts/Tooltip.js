...
var Tooltip = {
	...
	init: function (activator)
	{
		...
		if (Tooltip.autoFollowMouse != true) {
			...
			// Stick the entire thing in the top of the Tooltip
    ...
    * @private
	...
	{
		if( forceShift && event.shiftKey == false) return;
	...
	_attachEvent: function (element, event)
	{
        ...
        if (event == "toggle") {
            ...
            if (element.addEventListener) {
                element.addEventListener("dblclick", function (e) { Tooltip.toggle(element, e, true); }, false);
            } else if (element.attachEvent) {
	            element.attachEvent('on' + "dblclick", function (e) { Tooltip.toggle(element, e, true); });
            }
